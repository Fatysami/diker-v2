...
import { Save, Plus, Trash2, Upload, Loader2, Image as ImageIcon, Construction } from "lucide-react";
...
const AdminStrassenbauTab = ({ 
  ...
  const uploadImage = async (file: File, sectionId?: string): Promise<string | null> => {
    ...
    const fileName = `strassenbau/${sectionId || crypto.randomUUID()}-${Date.now()}.${fileExt}`;
