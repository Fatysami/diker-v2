...
import { Save, Plus, Trash2, Loader2, Image as ImageIcon, Construction, Info } from "lucide-react";
...
interface AdminStrassenbauTabProps {
  sections: StrassenbauSection[];
  setSections: React.Dispatch<React.SetStateAction<StrassenbauSection[]>>;
  saving: boolean;
...
const iconOptions = [
  ...
  { value: "MessageSquare", label: "Beratung" },
  { value: "Settings", label: "Projekte" },
  { value: "Construction", label: "Baustelle" },
  { value: "Truck", label: "LKW" },
];

const AdminStrassenbauTab = ({ 
  sections, 
  setSections, 
  saving, 
  setSaving,
  onRefresh 
}: AdminStrassenbauTabProps) => {
  ...
  const handleSectionChange = (id: string, field: keyof StrassenbauSection, value: string | boolean | number | string[]) => {
    setSections(prev =>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="col-span-2 row-span-3">
                      <ImageUploadWithFocalPoint
                        ...
                        label="Hauptbild"
                        isLarge
                        storagePath="strassenbau"
                        tableName="strassenbau_sections"
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          id={`active-strassenbau-${section.id}`}
                          checked={section.is_active}
                          onChange={(e) => handleSectionChange(section.id, "is_active", e.target.checked)}
                          className="w-4 h-4"
                        />
                        <label htmlFor={`active-strassenbau-${section.id}`} className="text-sm text-muted-foreground">
                          Aktiv
                        </label>
                      </div>
                      <div className="flex items-center gap-2">
...
export default AdminStrassenbauTab;
