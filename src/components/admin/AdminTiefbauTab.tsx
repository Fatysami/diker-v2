...
import { Save, Plus, Trash2, Upload, Loader2, Image as ImageIcon, HardHat } from "lucide-react";
...
interface AdminTiefbauTabProps {
  sections: TiefbauSection[];
  setSections: React.Dispatch<React.SetStateAction<TiefbauSection[]>>;
  saving: boolean;
...
const iconOptions = [
  ...
  { value: "Settings", label: "Reparatur" },
  { value: "HardHat", label: "Baustelle" },
  { value: "Truck", label: "LKW" },
];

type ImageField = 'image_url' | 'image_url_2' | 'image_url_3' | 'image_url_4';

const AdminTiefbauTab = ({ 
  sections, 
  setSections, 
  saving, 
  setSaving,
  onRefresh 
}: AdminTiefbauTabProps) => {
  ...
  const handleSectionChange = (id: string, field: keyof TiefbauSection, value: string | boolean | number | string[]) => {
    setSections(prev =>
  ...
  const uploadImage = async (file: File, sectionId?: string): Promise<string | null> => {
    ...
    const fileName = `tiefbau/${sectionId || crypto.randomUUID()}-${Date.now()}.${fileExt}`;
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          id={`active-tiefbau-${section.id}`}
                          checked={section.is_active}
                          onChange={(e) => handleSectionChange(section.id, "is_active", e.target.checked)}
                          className="w-4 h-4"
                        />
                        <label htmlFor={`active-tiefbau-${section.id}`} className="text-sm text-muted-foreground">
                          Aktiv
                        </label>
                      </div>
                      <div className="flex items-center gap-2">
...
export default AdminTiefbauTab;
